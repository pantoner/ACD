...
class MainWindow(QMainWindow, Ui_MainWindow):
	...
	def getevents(self):
		...
		if not df.empty:
			thisdict = (lambda x, y:{'event':[x],'thistime':df[y].values[0][11:14] + df[y].values[0][14:16]})('aup','timeofaup' )
		...
		df = df.loc[df['symbol'] ==symbol]
		if not df.empty:
			thisdict = (lambda x, y:{'event':[x],'thistime':df[y].values[0][11:14] + df[y].values[0][14:16]})('adwn','timeofadwn' )
			thisdf = pd.DataFrame(thisdict)
			thisdf.to_sql('events', conn2, if_exists='append')
